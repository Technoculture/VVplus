...
const TemplateCard = ({ handleClickOnNewDesign }: NewDesignProps) => {
  ...
  return (
    <main className="flex flex-col relative  left-[50%] translate-x-[-50%]   md:my-8 md:gap-4 my-0 gap-2  justify-center items-center ">
        ...
        {cards.map((e) => {
          return (
           ...
           `}
            >
              <div
                className="w-[190px] bg-zinc-300 relative object-fill overflow-hidden h-[300px] border-[1px] border-amber-300 rounded-[12px]"
      ...
      ${isEuropaCardOpen === true ? "mt-[4vh]" : ""}
      `}
      >
        <span className="italic select-none font-Roboto text-sky-500">
          Learn more
        </span>{" "}
        <span className="select-none font-Roboto">about this model</span>
      </div>
    </main>
  );
};
